...
- **document-writer** — bounded creation and editing of formal product and engineering documents (PRDs, technical solutions, architecture/interface designs, delivery documents, ADRs), dispatched with one explicit mode: `MODE: WRITE` for creation/body edits, `MODE: REVIEW` for expression, structure, document-internal consistency, audience fit, and readability (returns `PASS | REJECT | BLOCKED` with findings bound to the inspected revision and published as board review records; never certifies protocol/technical semantics). Returns a result with the content location and a retrievable revision (or readable retained snapshot) plus its hash for every substantive edit; a plain `DONE` excludes independent review and publication.
...
- **Oracle boundaries** — `prompts/oracle_append.md` makes Oracle review dispatched documents for correctness, consistency, feasibility, acceptance fit, and audience fit, and answer `PASS | REJECT | BLOCKED` with findings bound to the inspected revision (or readable retained snapshot) plus SHA-256; each verdict is published as a board review record and the full `bb://` ID returned. On rereview, prior findings get explicit dispositions (adopted/verified, adopted/unverified, not adopted, deferred/disputed) and review versions link with `related`, never `supersedes`. Oracle never issues publication or execution authorization on its own.
- **Council, Orchestrator, and Fixer prompts** — `prompts/council_append.md` adds evidence-and-disagreement judgment to the Council synthesis agent (judge claims by traceable current-version evidence, attribute reported checks to their seats, retain material dissent; in omo-slim 2.2.21 the append reaches the synthesis agent, not automatically every generated councillor seat); `prompts/orchestrator_append.md` adds the document/review gating policy and mixed-task routing guidance (ordinary document work is accepted after the writer's final full-document self-check — `DONE` excludes independent review and publication; Oracle review is required when requested or for impact-gated categories; each first review starts in a fresh session, rereviews may reuse the reviewer session; one automatic corrective round and rereview per work objective across all tracks and sessions; route each part of a mixed request by its own needs and stage delegation when specialization gains outweigh handoff costs; resolve internal bookkeeping labels into user-understandable references before replying); `prompts/fixer_append.md` gives the built-in fixer a final board-handoff step — publish or reuse one task-level handoff with its full `bb://` ID, including for blocked or no-change outcomes — which carries the evidence contract now that local Lark skill files (including `lark-skill-maker` work) route to fixer.
...
prompts/
  ...
  fixer_append.md          Board-handoff rule appended to the built-in fixer role
