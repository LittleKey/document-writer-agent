...
Copy `prompts/document-writer.md`, `prompts/lark-operator.md`, `prompts/oracle_append.md`, `prompts/council_append.md`, `prompts/orchestrator_append.md`, and `prompts/fixer_append.md` into the prompt location determined in step 2:
...
- Before claiming activation, confirm per step 2.7 that the effective config holds no unreconciled `agents.<name>.prompt` inline override for `document-writer`, `lark-operator`, `oracle`, `council`, `orchestrator`, or `fixer`. An inline override wins over the copied file; reconcile it first, and do not claim the prompts are active while one remains.

`oracle_append.md` is what enables the Oracle document-review contract: it makes Oracle review dispatched documents for correctness, consistency, feasibility, acceptance fit, and audience fit, bind findings to the inspected revision (or a readable retained snapshot with SHA-256), publish each verdict as a board review record returning the full `bb://` ID, and answer `PASS | REJECT | BLOCKED` — never a bare narrative verdict, and never publication/execution authorization. `document-writer.md` carries the dispatched-mode contract (`MODE: WRITE` / `MODE: REVIEW`; REVIEW returns `PASS | REJECT | BLOCKED` with findings published as board review records, and refuses sessions that authored the target); `fixer_append.md` gives the built-in fixer the final board-handoff step (publish or reuse one task-level handoff with its full `bb://` ID, including for blocked or no-change outcomes). `council_append.md` adds evidence-and-disagreement judgment to the Council synthesis agent (judge claims by traceable current-version evidence, attribute reported checks to their seats, retain material dissent; in omo-slim 2.2.21 the append reaches the synthesis agent, not automatically every generated councillor seat); `orchestrator_append.md` adds the document/review gating policy (ordinary document work is accepted after the writer's final full-document self-check — `DONE` excludes independent review and publication; Oracle review when requested or impact-gated; fresh session per first review; one automatic corrective round and rereview per work objective) and mixed-task routing guidance (route mixed requests by each part's needs; stage delegation when specialization gains outweigh handoff costs).
